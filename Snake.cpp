...
void Snake::Reset()
{
    ...
    m_lost = false;
    SoundBuffer buffer;
buffer.loadFromFile("metalLatch.wav");
...
void Snake::IncreaseScore()
{
    sound.play();
    m_score += 10;
    	m_log->Add("Ты зохавал яблоко! Набрано очков: "
...
void Snake::Lose()
{
    m_log->Add("Ты просрал все полимеры! Начинай сначала.");
    m_lost = true;
}
...
void Snake::Cut(int l_segments)
{
    ...
    if (!m_lives)
    {
        ...
        return;
        	...
        	m_log->Add("Ты потерял одну жизнь! Осталось жизней: "
